...
export default function EditTourPage() {
  ...
  useEffect(() => {
    async function load() {
      const [tourRes, regionsRes] = await Promise.all([
        apiGet<any>(`/products/tours/${id}`),
        apiGet<any>('/products/regions'),
      ]);
      if (tourRes.success) {
        const t: any = tourRes.data;
        setTitle(t.title ?? '');
        setSlug(t.slug ?? '');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await apiPatch(`/products/tours/${id}`, body);
    setLoading(false);
    if (res.success) router.push('/dashboard/products');
    else setError('error' in res ? res.error.message : 'Failed to update tour');
  }

  if (fetching) return <div className="font-body text-ht-soft">Loading tour...</div>;
  ...
  return (
    <div>
      <div className="mb-6 sm:mb-8">
        ...
        <h1 className="font-display font-bold text-2xl sm:text-3xl text-ht-text mt-2">Edit tour</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={regionId} disabled
                className="w-full px-4 py-3 rounded-xl bg-ht-ink border border-ht-border text-ht-soft font-body text-sm focus:outline-none min-h-[44px] opacity-60 cursor-not-allowed">
                <option value="">—</option>
                {regions.map((r) => <option key={r.id} value={r.id}>{r.country?.name ? `${r.country.name} — ${r.name}` : r.name}</option>)}
              </select>
              <p className="mt-1 text-[10px] font-mono text-ht-muted">Region is locked after creation.</p>
            </div>
            <div>
