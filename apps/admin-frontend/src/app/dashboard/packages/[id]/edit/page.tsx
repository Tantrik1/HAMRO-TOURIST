...
export default function EditPackagePage() {
  ...
  useEffect(() => {
    async function load() {
      const [pkgRes, regionsRes] = await Promise.all([
        apiGet<any>(`/products/packages/${id}`),
        apiGet<any>('/products/regions'),
      ]);
      if (pkgRes.success) {
        const p: any = pkgRes.data;
        setTitle(p.title ?? '');
        setSlug(p.slug ?? '');
        setDescription(p.description ?? '');
        setTotalDays(Number(p.totalDays ?? 1));
        setBasePrice(Number(p.basePrice ?? 0));
        setSortOrder(p.sortOrder ?? 0);
        setStatus((p.status as 'draft' | 'published') ?? 'draft');
        setSelectedRegionIds(Array.isArray(p.regions) ? p.regions.map((r: any) => r.id) : []);
        setHighlights(arrayToLines(p.highlights));
        setInclusions(arrayToLines(p.inclusions));
        setExclusions(arrayToLines(p.exclusions));
        setItinerary(Array.isArray(p.itinerary) ? p.itinerary.map((d: any, i: number) => ({
        ...
        setMedia(mediaStateFromApi(p.media ?? (p.coverImageUrl ? { bannerImage: p.coverImageUrl } : null)));
        setSeo(seoStateFromApi(p.seo));
        setFaqs(Array.isArray(p.faqs) ? p.faqs.map((f: any) => ({
          question: f.question, answer: f.answer,
          sortOrder: f.sortOrder ?? 0, isActive: f.isActive ?? true,
        })) : []);
        setDiscounts(Array.isArray(p.groupDiscounts) ? p.groupDiscounts.map((g: any) => ({
  ...
  function toggleRegion(rid: string) {
    setSelectedRegionIds((prev) => prev.includes(rid) ? prev.filter((r) => r !== rid) : [...prev, rid]);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    body.regionIds = selectedRegionIds;
    ...
    const res = await apiPatch(`/products/packages/${id}`, body);
    setLoading(false);
    if (res.success) router.push('/dashboard/products');
    else setError('error' in res ? res.error.message : 'Failed to update package');
  }

  if (fetching) return <div className="font-body text-ht-soft">Loading package...</div>;
  ...
  return (
    <div>
      <div className="mb-6 sm:mb-8">
        ...
        <h1 className="font-display font-bold text-2xl sm:text-3xl text-ht-text mt-2">Edit package</h1>
