...
export default function EditActivityPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      const [activityRes, regionsRes] = await Promise.all([
        apiGet(`/products/activities/${id}`),
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.success) {
      ...
      setError('error' in res ? res.error.message : 'Failed to update activity');
    }
  }
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="font-display font-bold text-3xl text-ht-text mt-2">Edit Activity</h1>
      </div>
